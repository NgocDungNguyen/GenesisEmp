$(document).ready(function() {
        ...
        if (data.length > 0) {
            data.forEach(function(item, index) {
                ...
                $row.append($('<td>').text(item.furnitureName))
                    ...
                    .append($('<td>').append($deleteButton).append($priorityButton));
            ...
            $tableBody.sortable({
                update: function(event, ui) {
                    console.log('Updated sort order:', $(this).sortable('toArray'));
                }
            });
    ...
    function sortTable(sortBy) {
        ...
        rows.sort((a, b) => {
            const valA = $(a).find('td').eq(sortBy === 'furnitureName' ? 0 : 1).text().toUpperCase();
            const valB = $(b).find('td').eq(sortBy === 'furnitureName' ? 0 : 1).text().toUpperCase();
            if (valA < valB) {
                return -1;
            }
            if (valA > valB) {
